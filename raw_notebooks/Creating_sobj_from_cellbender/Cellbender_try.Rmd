...
for (name in library_list){
  ...
  file_path = file.path(this_base_path, name, CELLBENDER_BASENAME)
    ...
    cellranger_filtered = filtered_dgc[, colnames(filtered_dgc) %in% colnames(counts_matrix)]
    
   
    counts_matrix_filtered = counts_matrix[, colnames(counts_matrix) %in% colnames(cellranger_filtered)]
   ...
   print("1")
    ...
    #counts_matrix_filtered = sum_duplicate_rownames_of_dgc_matrix(counts_matrix_filtered) #silence again
    print(dim(counts_matrix_filtered))
   print("1.5")
    # note: for now we are identifying genes with duplicated rownames and dropping them
    n_occur = data.frame(table(rownames(counts_matrix_filtered)))
    repeated_rownames = n_occur[n_occur$Freq > 1,]$Var1
    counts_matrix_filtered = counts_matrix_filtered[!rownames(counts_matrix_filtered) %in% repeated_rownames]
    print(dim(counts_matrix_filtered))
   print("2")
   ...
   print("3")
    ...
    #cellbender_sobj = CreateSeuratObject(counts_matrix_filtered, metadata)
   # qsave(cellbender_sobj, cellbender_seurat_path) 
  
  
  
  sobj_new = CreateSeuratObject(counts=counts_matrix_filtered, meta.data=metadata)
  seurat_object_list[[name]] = sobj_new
  
  qsfilepath <- paste0("output/", name, ".qs") #output folder
  qsave(sobj_new, file = qsfilepath)  
