...
```{r}
caudate_sobj = qread("Cellbender_seurat/Bennett_caudate_clean.qs")
putamen_sobj = qread("Cellbender_seurat/Bennett_putamen_clean.qs")
...
# Define the genes and clusters you want to plot
features <- c("ALDH1A1", "AQP4","GFAP", "GINS3","C3", "CFB","S100A10", "GRIA2", "CSPG5", "VCAN", "pct_mito") 
...
```{r}
Idents(putamen_astrocyte) = putamen_astrocyte$RNA_snn_res.0.2
markers = FindAllMarkers(putamen_astrocyte, only.pos = TRUE)

num_clusters = length(unique(Idents(putamen_astrocyte))) - 1
...
```{r}
Idents(putamen_astrocyte) <- "RNA_snn_res.0.2"
...
```{r}
classes = c("GRIA2","GFAP", "GFAP", "GFAP", "GFAP", "GFAP")

putamen_astrocyte= assignCellClasses(putamen_astrocyte, classes=classes, cluster_col="RNA_snn_res.0.2", class_col = "cell_class")
...
markers = FindAllMarkers(caudate_microglia, only.pos = TRUE)

num_clusters = length(unique(Idents(caudate_microglia))) - 1
...
```{r}
head(caudate_microglia)
```



```{r}
Idents(caudate_microglia) <- "RNA_snn_res.0.2"


# Define the genes and clusters you want to plot
features <- c("CD14", "CD40", "CD86", "CCL5", "CCL20", "CXCL1","CD163","BDNF", "CSF1","C1QA", "C1QB", "CX3CR1", "P2RY12")
...
markers = FindAllMarkers(caudate_immune, only.pos = TRUE)
...
```{r}
Idents(caudate_immune) <- "RNA_snn_res.1"


# Define the genes and clusters you want to plot
features <- c("CD3D", "CD3E", "CD3G","CD4","CD8A", "CD8B","FOXP3", "CD96", "RGS10", "SKAP1", "CD69", "NKG7") 
...
caudate_immune= assignCellClasses(caudate_immune, classes=classes, cluster_col="RNA_snn_res.0.2", class_col = "cell_class")
...
```{r}
DimPlot(caudate_immune, group.by = "RNA_snn_res.0.2")
DimPlot(caudate_immune, group.by = "RNA_snn_res.1")
DimPlot(caudate_immune, group.by = "library")
DimPlot(caudate_immune, group.by = "donor_id")
DimPlot(caudate_immune, group.by = "Condition")
```
...
DimPlot(caudate_immune, group.by = "RNA_snn_res.0.8")
DimPlot(caudate_immune, group.by = "RNA_snn_res.0.9")
DimPlot(caudate_immune, group.by = "RNA_snn_res.1")
```
