...
ggplot(sobj_df, aes(x = Taf1, y = sobj_avg_exp)) +
  geom_point() +
  geom_point(aes(x = Taf1, y = sobj_avg_exp), color = "red") + 
  labs(
    x = "Taf1 correlation",
...
```{r}
Taf1corr(XDP_SPN, "XDP SPN")
Taf1corr(Control_SPN, "Control SPN")
Taf1corr(XDP_Interneuron, "XDP interneuron")
Taf1corr(Control_Interneuron, "Control interneuron")
```
...
```{r}
Taf1corrdf(XDP_SPN, "GSEA/de/caudate_neurons/XDP_SPN_df.csv")
Taf1corrdf(Control_SPN, "GSEA/de/caudate_neurons/Control_SPN_df.csv")
Taf1corrdf(XDP_Interneuron, "GSEA/de/caudate_neurons/XDP_interneuron_df.csv")
Taf1corrdf(Control_Interneuron, "GSEA/de/caudate_neurons/Control_interneuron_df.csv")
```

```{r}
XDP_SPN_df = read.csv("GSEA/de/caudate_neurons/XDP_SPN_df.csv")
Control_SPN_df = read.csv("GSEA/de/caudate_neurons/Control_SPN_df.csv")
XDP_interneuron_df = read.csv("GSEA/de/caudate_neurons/XDP_interneuron_df.csv")
Control_interneuron_df = read.csv("GSEA/de/caudate_neurons/Control_interneuron_df.csv")

XDP_SPN_df$X = NULL
Control_SPN_df$X = NULL
XDP_interneuron_df$X = NULL
Control_interneuron_df$X = NULL

XDP_SPN_df
Control_SPN_df
XDP_interneuron_df
Control_interneuron_df
```
...
#example
```{r}
outpath = file.path(BASE_PATH, DE_SUBDIR, "final_gsea")
...
for (cell_type in caudate_neurons_types) {
  ...
  gsea_file_name <- paste("~/GSEA/de/caudate_neurons/final_gsea/", cell_type, ".csv__gsea_taf1correlation.tsv", sep = "")
...
#without taf1
```{r}
Dot_plot(XDP_SPN_gsea_df, "XDP SPNs: Top 10 Gene Sets by NES +/-")
Dot_plot(Control_SPN_gsea_df, "Control SPNs: Top 10 Gene Sets by NES +/-")
Dot_plot(XDP_interneuron_gsea_df, "XDP Interneurons: Top 10 Gene Sets by NES +/-")
Dot_plot(Control_interneuron_gsea_df, "Control Interneurons: Top 10 Gene Sets by NES +/-")
```
